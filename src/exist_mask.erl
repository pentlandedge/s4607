...
new(Fields) when is_list(Fields) ->
    #exist_mask{
        ...
        sensor_lon = 1,
        sensor_alt = 1,
        lat_scale_factor = bool_to_int(lists:member(lat_scale_factor, Fields)),
        lon_scale_factor = bool_to_int(lists:member(lon_scale_factor, Fields)),
        spu_along_track = bool_to_int(lists:member(spu_along_track, Fields)),
        spu_cross_track = bool_to_int(lists:member(spu_cross_track, Fields)),
        spu_alt = bool_to_int(lists:member(spu_alt, Fields)),
        sensor_track = bool_to_int(lists:member(sensor_track, Fields)),
        sensor_speed = bool_to_int(lists:member(sensor_speed, Fields)),
        sensor_vert_vel = bool_to_int(lists:member(sensor_vert_vel, Fields)),
        sensor_track_unc = bool_to_int(lists:member(sensor_track_unc, Fields)),
        sensor_speed_unc = bool_to_int(lists:member(sensor_speed_unc, Fields)),
        sensor_vert_vel_unc = bool_to_int(lists:member(sensor_vert_vel_unc, Fields)),
        platform_heading = bool_to_int(lists:member(platform_heading, Fields)),
        platform_pitch = bool_to_int(lists:member(platform_pitch, Fields)),
        platform_roll = bool_to_int(lists:member(platform_roll, Fields)),
        dwell_center_lat = 1,
        dwell_center_lon = 1,
        dwell_range_half_extent = 1,
        dwell_angle_half_extent = 1,
        sensor_heading = bool_to_int(lists:member(sensor_heading, Fields)),
        sensor_pitch = bool_to_int(lists:member(sensor_pitch, Fields)),
        sensor_roll = bool_to_int(lists:member(sensor_roll, Fields)),
        mdv = bool_to_int(lists:member(mdv, Fields)),
        mti_report_index = bool_to_int(lists:member(mti_report_index, Fields)),
        target_hr_lat = bool_to_int(lists:member(target_hr_lat, Fields)),
        target_hr_lon = bool_to_int(lists:member(target_hr_lon, Fields)),
        target_delta_lat = bool_to_int(lists:member(target_delta_lat, Fields)),
        target_delta_lon = bool_to_int(lists:member(target_delta_lon, Fields)),
        geodetic_height = bool_to_int(lists:member(geodetic_height, Fields)),
        target_vel_los = bool_to_int(lists:member(target_vel_los, Fields)),
        target_wrap_velocity = bool_to_int(lists:member(target_wrap_velocity, Fields)),
        target_snr = bool_to_int(lists:member(target_snr, Fields)),
        target_classification = bool_to_int(lists:member(target_classification, Fields)),
        target_class_prob = bool_to_int(lists:member(target_class_prob, Fields)),
        target_slant_range_unc = bool_to_int(lists:member(target_slant_range_unc, Fields)),
        target_cross_range_unc = bool_to_int(lists:member(target_cross_range_unc, Fields)),
        target_height_unc = bool_to_int(lists:member(target_height_unc, Fields)),
        target_rad_vel_unc = bool_to_int(lists:member(target_rad_vel_unc, Fields)),
        truth_tag_app = bool_to_int(lists:member(truth_tag_app, Fields)),
        truth_tag_entity = bool_to_int(lists:member(truth_tag_entity, Fields)),
        target_rcs = bool_to_int(lists:member(target_rcs, Fields))}.
